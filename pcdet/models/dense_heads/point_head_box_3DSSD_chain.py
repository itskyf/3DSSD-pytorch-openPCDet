...
class PointHeadBox3DSSDChain(PointHeadTemplate):
    """
    ...
    def __init__(self, num_class, input_channels, model_cfg, predict_boxes_when_training=False, **kwargs):
        ...
        self.dir_head = nn.Linear(mid_channels, self.box_coder.code_size - 6)
        ...
        self.cls_head = nn.Sequential(
            ...
            nn.LayerNorm(mid_channels),
            nn.ReLU(),
            nn.Linear(mid_channels, num_class),
        )
    ...
    def forward(self, batch_dict):
        ...
        xyz_feat = self.xyz_conv(center_features)
        xyz = self.xyz_head(xyz_feat)

        dir_feat = torch.cat([center_features, xyz_feat], dim=-1)
        dir_feat = self.dir_conv(dir_feat)
        dir_pred = self.dir_head(dir_feat)

        whl_feat = torch.cat([center_features, dir_feat], dim=-1)
        whl_feat = self.whl_conv(whl_feat)
        whl = self.whl_head(whl_feat)

        cls_feat = torch.cat([center_features, whl_feat], dim=-1)
        center_cls_preds = self.cls_head(cls_feat)
        center_box_preds = torch.cat([xyz, whl, dir_pred], dim=-1)
